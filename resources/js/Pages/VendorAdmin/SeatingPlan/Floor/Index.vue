<template>
      ...
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            <h1 class="header-title">Floors</h1>
            <p class="header-subtitle">Manage branch floors.</p>
          </div>
          <button type="button" v-if="can('floors.create')" class="btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus" />
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Create <span class="create-text">Floor</span>
            </span>
          </button>
        </div>
      </div>
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this floor?" cancel-label="Keep Floor" confirm-label="Delete Floor"
...
const tableId = 'floorTable'
const dtRef = ref(null)
const datatableUrl = computed(() => route('vendor.floors.getdata'))
...
const goCreate = () => router.visit(route('vendor.floors.create'))
const goEdit = (id) => router.visit(route('vendor.floors.edit', id))
...
function confirmDelete() {
  ...
  if (!id) return
  deleting.value = true
  router.delete(route('vendor.floors.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
...
/* Preserve server-rendered activation badges */
:deep(.status-badge) {
  display: inline-flex;
  align-items: center;
  border-radius: 8px;
