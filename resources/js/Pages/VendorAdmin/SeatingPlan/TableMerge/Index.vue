<template>
      ...
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            <h1 class="header-title">Table Merges</h1>
            <p class="header-subtitle">Manage merged tables.</p>
          </div>
          <button v-if="can('table-merges.create')" type="button" class="btn btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus" />
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Create <span class="create-text">Merge</span>
            </span>
          </button>
        </div>
      </div>
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this merge?" cancel-label="Keep Merge" confirm-label="Delete Merge"
...
const tableId = 'tableMergeTable'
const dtRef = ref(null)
const datatableUrl = computed(() => route('vendor.table-merges.getdata'))
...
const goCreate = () => router.visit(route('vendor.table-merges.create'))
const goEdit = (id) => router.visit(route('vendor.table-merges.edit', id))

const closeMerge = (id) => {
  if (window.confirm('Close this merge?'))
    router.patch(route('vendor.table-merges.close', id), {}, { preserveScroll: true })
}
...
function confirmDelete() {
  ...
  if (!id) return
  deleting.value = true
  router.delete(route('vendor.table-merges.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
...
function bindActions() {
  ...
  $(document).on('click', `${s} .js-close`, (e) => closeMerge(e.currentTarget.dataset.id))
...
:deep(.merge-type--billing) {
  color: #ef4444;
}

:deep(.merge-type--capacity) {
  color: #10b7f4;
}
