<template>
      ...
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            ...
            <p class="header-subtitle">Manage your suppliers, contact details and branch assignments.</p>
          </div>
          <button v-if="can('suppliers.create')" type="button" class="btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus" />
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Create <span class="create-text">Supplier</span>
            </span>
          </button>
        </div>
      </div>
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this supplier?" cancel-label="Keep Supplier" confirm-label="Delete Supplier"
...
const datatableUrl = computed(() => route('vendor.suppliers.getdata'))
...
const columns = computed(() => ([
  {
    ...
    render: (data, type, row) => `
      <div>
        <div class="fw-bold text-dark text-nowrap">${data}</div>
        <div class="text-muted x-small">SUP-${row.id}</div>
      </div>
    `
  },
  {
...
function goCreate() {
  router.visit(route('vendor.suppliers.create'))
}

function goEdit(id) {
  router.visit(route('vendor.suppliers.edit', id))
}
...
function confirmDelete() {
  ...
  if (!id) return
  deleting.value = true
  router.delete(route('vendor.suppliers.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
