<template>
      ...
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            ...
            <p class="header-subtitle">Manage ingredients, stock levels, units and cost per unit.</p>
          </div>
          <button v-if="can('ingredients.create')" type="button" class="btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus-lg"></i>
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Create <span class="create-text">Ingredient</span>
            </span>
          </button>
        </div>
      </div>
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this ingredient?" cancel-label="Keep Ingredient" confirm-label="Delete Ingredient"
...
const tableId = 'ingredientTable'
const dtRef = ref(null)

const datatableUrl = computed(() => route('vendor.ingredients.getdata'))
...
const columns = computed(() => ([
  {
    ...
    render: (data, type, row) => `
      <div>
        ...
        <div class="text-muted x-small">ING-${row.id}</div>
      </div>
    `
  },
  {
  ...
  },
  {
    ...
    name: 'current_stock',
    render: (data) => `<span class="stock-chip">${data}</span>`
  },
  {
    data: 'alert_quantity',
    name: 'alert_quantity',
...
function goCreate() {
  router.visit(route('vendor.ingredients.create'))
}

function goEdit(id) {
  router.visit(route('vendor.ingredients.edit', id))
}
...
function confirmDelete() {
  ...
  if (!id) return
  deleting.value = true
  router.delete(route('vendor.ingredients.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
