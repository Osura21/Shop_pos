<template>
    ...
    <div class="card-modern">
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            ...
            <p class="header-subtitle">Manage all menus for this tenant.</p>
          </div>
          <button v-if="can('menus.create')" type="button" class="btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus" />
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Create <span class="create-text">Menu</span>
            </span>
          </button>
        </div>
      </div>
      ...
      <div class="table-container-modern">
        <DataTable ref="dtRef" :id="tableId" :url="datatableUrl" :columns="columns" :columnDefs="columnDefs"
          :order="[[1, 'desc']]" searchPlaceholder="Search menu name">
          <template #header>
            <tr>
              <th>Name</th>
              <th>Branch</th>
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this menu?" cancel-label="Keep Menu" confirm-label="Delete Menu"
...
const datatableUrl = computed(() => route('vendor.menus.getdata'))
...
function goCreate() {
  router.visit(route('vendor.menus.create'))
}

function goEdit(id) {
  router.visit(route('vendor.menus.edit', id))
}
...
function confirmDelete() {
  ...
  if (!id) return
  deleting.value = true
  router.delete(route('vendor.menus.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
