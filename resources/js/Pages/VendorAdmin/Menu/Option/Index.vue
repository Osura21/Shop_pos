<template>
      ...
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            ...
            <p class="header-subtitle">Manage reusable product option templates.</p>
          </div>
          <button v-if="can('options.create')" type="button" class="btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus" />
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Create <span class="create-text">Option</span>
            </span>
          </button>
        </div>
      </div>
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this option?" cancel-label="Keep Option" confirm-label="Delete Option"
...
const tableId = 'optionTable'
const dtRef = ref(null)
const datatableUrl = computed(() => route('vendor.options.getdata'))
...
function goCreate() {
  router.visit(route('vendor.options.create'))
}

function goEdit(id) {
  router.visit(route('vendor.options.edit', id))
}
...
function confirmDelete() {
  ...
  if (!id) return
  deleting.value = true
  router.delete(route('vendor.options.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
