<template>
    ...
    <div class="card-modern">
      <div class="card-modern-header">
        <div class="header-content">
          <div class="header-title-group">
            ...
            <p class="header-subtitle">Manage staff accounts, roles, and status.</p>
          </div>
          <button v-if="can('staff-member.create')" type="button" class="btn-primary-modern" @click="goCreate">
            <i class="bi bi-plus" />
            <span class="d-inline-flex align-items-center gap-1 text-nowrap">
              Add <span class="create-text">Staff</span>
            </span>
          </button>
        </div>
      </div>
      ...
      <div class="table-container-modern">
        <DataTable ref="dtRef" :id="tableId" :url="datatableUrl" :columns="columns" :columnDefs="columnDefs"
          :order="[[5, 'desc']]" searchPlaceholder="Search name, email or phone" :showKebabDropdown="true"
  ...
  <DeleteModal v-model:show="showDeleteModal" :target-id="deleteTarget.id" :target-name="deleteTarget.name"
    :loading="deleting" title="Delete this staff account?" cancel-label="Keep Staff" confirm-label="Delete Staff"
...
const datatableUrl = computed(() => route('settings.staff.getdata'))
...
function goCreate() {
  router.visit(route('settings.staff.create'))
}

function goEdit(id) {
  router.visit(route('settings.staff.edit', id))
}

function toggleStatus(id) {
  router.post(route('settings.staff.status.toggle', id), {}, { preserveScroll: true })
}

function handleKebabClick(action, button) {
  console.log(`Kebab button clicked: ${action}`, button)
}
...
function confirmDelete() {
  ...
  router.delete(route('settings.staff.destroy', id), {
    preserveScroll: true,
    onSuccess: () => {
...
function bindActions() {
  ...
  $(document).on('click', `${selector} .js-toggle`, (e) => {
    const id = e.currentTarget?.dataset?.id
...
function unbindActions() {
  ...
  $(document).off('click', `${selector} .js-toggle`)
