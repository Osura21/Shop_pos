...
const brandNewCars = ref([])
const loading = ref(true)

const detailUrl = (car) => {
    if (car?.slug) return route("vendorsite.product", { slug: car.slug })
...
async function fetchFeatured() {
    loading.value = true
    try {
        const res = await axios.get(route("vendorsite.vehicles.brand-new"))
        brandNewCars.value = res.data ?? []
    } catch (e) {
        console.error(e)
        brandNewCars.value = []
    } finally {
...
<template>
    <section v-if="brandNewCars.length || loading" class="py-5 bg-white">
        <div class="container py-5">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end gap-4 mb-5">
                <div>
                    ...
                    <h2 class="fw-bold display-7 mb-0">Brand New Vehicles Listings</h2>
                </div>

                <Link
                    :href="route('vendorsite.vehicles.index', { condition: 'Brand New' })"
            ...
            <div v-else class="row g-4">
                <div v-for="(car, index) in brandNewCars" :key="car.id" class="col-12 col-md-6 col-lg-3">
                    <Link
                        :href="detailUrl(car)"
