<template>
  <div class="mv-list">
    <input class="mv-miniInput" v-model="search" placeholder="Search makes..." />

    <label v-for="m in filtered" :key="m.id" class="mv-check">
      <input class="mv-check__in" type="checkbox" :value="String(m.id)" v-model="form.brand" @change="apply" />
    ...
    <div v-if="!filtered.length" class="mv-smallEmpty">No makes</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    setFormData() {
      ...
      if (q.hasOwnProperty("brand")) this.form.brand = Array.isArray(q.brand) ? q.brand.map(String) : [String(q.brand)];
    },
    apply() {
      const q = this.$page.props.requestQuery || {};
      // Clear model when make changes (prevents invalid model selection)
      router.get(window.location.pathname, { ...q, page: 1, brand: this.form.brand, model: [] }, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
