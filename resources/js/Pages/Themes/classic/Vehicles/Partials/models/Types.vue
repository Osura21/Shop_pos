<template>
  <div class="mv-list">
    <input class="mv-miniInput" v-model="search" placeholder="Search types..." />

    <label v-for="t in filtered" :key="t.id" class="mv-check">
      <input class="mv-check__in" type="checkbox" :value="String(t.id)" v-model="form.type" @change="apply" />
      <span class="mv-check__box">
        <i class="fa-solid fa-check mv-check__icon"></i>
      </span>
      <span class="mv-check__txt">{{ t.title }}</span>
      <span class="mv-check__count" v-if="t.vehicles_count">
        {{ t.vehicles_count }}
      </span>
    </label>

    <div v-if="!filtered.length" class="mv-smallEmpty">No types</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    setFormData() {
      ...
      if (q.hasOwnProperty("type")) this.form.type = Array.isArray(q.type) ? q.type.map(String) : [String(q.type)];
    },
    apply() {
      const q = this.$page.props.requestQuery || {};
      router.get(window.location.pathname, { ...q, page: 1, type: this.form.type }, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
