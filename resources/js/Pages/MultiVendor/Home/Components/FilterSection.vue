    ...
    <section id="filter" class="filter-overlap-section">
        <div class="container">
            <div class="filter-shell step" ref="filterWrap">
                <!-- <div class="filter-shell__header text-center">
                    <span class="badge rounded-pill custom-bg-primary bg-opacity-10 custom-text-primary px-4 py-2 mb-3">
                        Browse Collection
                    </span>
                    <h2 class="fw-bold mb-2">Find your next vehicle</h2>
                    <p class="text-muted mb-0">
                        Browse all vehicle types and filter below.
                    </p>
                </div> -->
                            ...
                            <div v-show="openPanel === 'price'" class="mv-ddPanel" @click.stop>
                                <div class="mv-ddBody mv-pricePanel">
                                    ...
                                    <div class="mv-rangeWrap">
                                        ...
                                        <input class="mv-range mv-range--min" type="range" :min="priceFloor"
                                            :max="priceCeil" :step="priceStep" v-model.number="priceMin"
                                            @input="onMinRange" />
                                        <input class="mv-range mv-range--max" type="range" :min="priceFloor"
                                            :max="priceCeil" :step="priceStep" v-model.number="priceMax"
                                            @input="onMaxRange" />
                                    </div>
            ...
            <LocationModal :open="isLocationModalOpen" :districts="districts" :cities-all="citiesAll"
                :selected-district-id="selectedDistrictId" :selected-city-id="selectedCityId" :total-posts="0"
                :active-brand-id="null" :active-model-id="null" @close="isLocationModalOpen = false"
                @apply="applyLocation" />
            ...
            <div v-if="isYearModalOpen" class="year-modal-backdrop" @click.self="closeYearModal">
                <div class="year-modal-card">
                    ...
                    <!-- <div class="year-posts-box mt-3">
                        <div class="year-posts-label">Posts available for selected range</div>
                        <div class="year-posts-count">
                            <span v-if="yearCountLoading">Checking...</span>
                            <span v-else>{{ yearRangePostCount }}</span>
                        </div>
                    </div> -->
...
const vehicleBrandId = ref(null)
const vehicleModelId = ref(null)
const brands = ref([])
const models = ref([])
...
const conditionSlug = ref(null)
const selectedTypeId = ref(null)

const selectedYearFrom = ref(null)
const selectedYearTo = ref(null)

const yearFromDraft = ref(null)
const yearToDraft = ref(null)
...
const priceMin = ref(priceFloor)
const priceMax = ref(priceCeil)
...
const locationLabel = computed(() => {
    ...
    const d = districts.value.find(x => x.slug === locationSlug.value)
    if (d) return d.name
    const c = citiesAll.value.find(x => x.slug === locationSlug.value)
    return c ? c.name : "All of Sri Lanka"
})
...
async function fetchYearRangeCount() {
    ...
    try {
        const conditionObj = conditionsFromProps.value.find(c => c.slug === conditionSlug.value)

        const response = await axios.get(route("multivendor.vehicles.count"), {
            params: {
                ...
                condition: conditionObj?.name || null,
...
async function fetchTypes() {
    ...
    try {
        const res = await axios.get(route("multivendor.types"))
...
async function fetchBrands() {
    const res = await axios.get(route("multivendor.brands"))
    brands.value = (res.data || []).map(b => ({ value: b.id, label: b.title }))
}
...
async function fetchModels(brandId) {
    ...
    const res = await axios.get(route("multivendor.models"), { params: { brand_id: brandId } })
    models.value = (res.data || []).map(m => ({ value: m.id, label: m.title }))
}

watch(vehicleBrandId, (id) => {
    vehicleModelId.value = null
    fetchModels(id).catch(console.error)
})
...
watch([selectedDistrictId, selectedCityId, vehicleBrandId, vehicleModelId, selectedTypeId, conditionSlug], () => {
    if (isYearModalOpen.value) {
        fetchYearRangeCount()
    }
})
...
function buildAdsPath() {
    ...
    if (selectedTypeId.value) {
        const t = types.value.find(x => x.value === selectedTypeId.value)
        if (t) parts.push(slugify(t.label))
    }

    if (locationSlug.value) parts.push(locationSlug.value)

    const brandObj = brands.value.find(b => b.value === vehicleBrandId.value)
    const modelObj = models.value.find(m => m.value === vehicleModelId.value)

    if (brandObj) parts.push(slugify(brandObj.label))
    if (modelObj) parts.push(slugify(modelObj.label))
    if (conditionSlug.value) parts.push(conditionSlug.value)

    return "/ads" + (parts.length ? "/" + parts.join("/") : "")
}
