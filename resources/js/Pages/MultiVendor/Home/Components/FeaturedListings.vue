...
async function fetchFeatured() {
    loading.value = true
    try {
        const res = await axios.get(route("multivendor.vehicles"), {
            params: { featured: 1, per_page: 6 },
        })

        featuredCars.value = res.data?.data ?? []
    } catch (e) {
        console.error(e)
        featuredCars.value = []
    } finally {
...
<template>
    <section v-if="featuredCars.length" class="py-5 bg-white">
