...
<head>
    <meta charset="UTF-8">
    <title>Waiter Copy #{{ $ticket->id }}</title>
    <style>
        @page {
            margin: 8px;
            size: 80mm auto;
...
<body>
    ...
    @endphp
    <div class="receipt">
        <div class="title">WAITER COPY</div>

        <div class="line"></div>

        <div class="order-no">#{{ $ticket->id }}</div>
        ...
        <table class="summary">
            ...
            <tr>
                <td>Due</td>
                <td class="right">{{ number_format($dueAmount, 3) }}</td>
            </tr>
        </table>

        <div class="line"></div>

        <div class="footer">Waiter Copy | #{{ $ticket->id }}</div>
    </div>
