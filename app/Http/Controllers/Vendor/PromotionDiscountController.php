...
class PromotionDiscountController extends Controller
{
    ...
    {
        return Inertia::render('VendorAdmin/Promotions/Discounts/Index');
    }

    public function getData(Request $request)
    {
        $rows = PromotionDiscount::query()
            ->with('branch:id,name')
            ->where('tenant_id', $this->tenantId())
            ->select('promotion_discounts.*');
    ...
    {
        return Inertia::render('VendorAdmin/Promotions/Discounts/CreateUpdate', array_merge($this->promotionFormProps(), [
            'discount' => null,
        ]));
    }
    ...
    public function store(Request $request)
    {
        ...
        PromotionDiscount::create($this->promotionPayload($validated));

        return redirect()->route('vendor.promotions.discounts.index')->with('success', 'Discount created successfully.');
    }

    public function edit($id)
    {
        $discount = PromotionDiscount::where('tenant_id', $this->tenantId())->findOrFail($id);
    ...
    {
        $discount = PromotionDiscount::where('tenant_id', $this->tenantId())->findOrFail($id);
        $validated = $request->validate($this->promotionRules($discount->id));
        $discount->update($this->promotionPayload($validated));

        return redirect()->route('vendor.promotions.discounts.index')->with('success', 'Discount updated successfully.');
    }
...
{
    PromotionDiscount::where('tenant_id', $this->tenantId())->findOrFail($id)->delete();

    return redirect()
        ->route('vendor.promotions.discounts.index')
